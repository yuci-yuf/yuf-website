...
2. **Firebase + auth/admin model** — `lib/firebase.ts` is the single initialization point, exporting `app`, `auth`, and `db` (Firestore). It guards against hot-reload re-init via `getApps()`. All config comes from `NEXT_PUBLIC_FIREBASE_*` env vars, so import from `@/lib/firebase` rather than calling `initializeApp` again. `firebase.json` provisions Firestore plus Auth (email/password + Google sign-in).
   ...
   **`firestore.rules` holds the production ruleset** (no expiry). It uses a single-admin model — its `isAdmin()` helper treats any signed-in user as the admin (`request.auth != null`), since the only Auth account is the admin (no public sign-up). Public CMS content (`events`, `eventCategories`, `gallery`) is world-readable but admin-only to write; visitors may `create` in `contactSubmissions`/`registrations` but only admins may read/update/delete. Edit the rules in `firestore.rules` and deploy them (e.g. `firebase deploy --only firestore:rules`). Note the rules' `isAdmin()` is broader than the app's client-side check in `contexts/AuthContext.tsx`, which additionally requires a doc at `admins/{uid}`.
